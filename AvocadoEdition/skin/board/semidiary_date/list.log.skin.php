		...
		<div class="ui-pic <?=$no_member_class?>"  >
			...
			<div class="pic-header">
				<p class="no">
					...
					<?//넘버 대신 날짜 출력 ?>
					<i class= 'no1'><?=date('Y.m.d', strtotime($list_item['wr_datetime']))?></i>
					...
					<?if(strstr($list_item['wr_option'],"secret")){?>
					&nbsp;&nbsp;<span class="co-secret highlight1">#secret</span>
					<?}else if ($list_item['wr_protect']!=''){?>
					&nbsp;&nbsp;<span class="co-secret highlight2">#protect</span>
					<?}else if($list_item['wr_secret'] == '1' ){?>
					&nbsp;&nbsp;<span class="co-member highlight3">#member only</span>
					<?}//@200602 ?>
			...
			<?} else { ?>
				...
				<? if($image_url) { ?>
					...
					<?	if($blind_class) { 
						// 블라인드 (19금 필터링)
					?>
					<a href="#" class="ui-remove-blind"><span>후방주의<br />클릭하면 보여요</span></a>
					<? } ?>
				<? } ?>
