...
<section id="bo_w">
	...
	<form name="fwrite" id="fwrite" action="<?php echo $action_url ?>" onsubmit="return fwrite_submit(this);"
		...
		if ($is_notice || $is_html || $is_secret || $is_mail) {
			...
			$option .= "\n" . '<input type="checkbox" id="dark" name="dark" value="dark" ' . $dark_checked . '>' . "\n" . '<label for="dark">배경추가</label>';
			...
			<tbody>
				...
				<?php if ($is_email) { ?>
					<tr>
						...
						<td><input type="text" name="wr_email" value="<?php echo $email ?>" id="wr_email"
								class="frm_input email" size="50" maxlength="100"></td>
					</tr>
				<?php } ?>
						...
						<td>
							...
							<?php if ($is_file_content) { ?>
								<input type="text" name="bf_content[]"
									...
									class="frm_file frm_input" size="50">
							<?php } ?>
		...
		<div class="btn_confirm">
			<button type="submit" id="btn_submit" accesskey="s" class="ui-btn point"><i class="material-icons">edit</i>
				작성</button>
			<a href="./board.php?bo_table=<?php echo $bo_table ?>" class="ui-btn"><i class="material-icons">list</i>
				목록</a>
		</div>
	</form>
