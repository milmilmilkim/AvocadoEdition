...
<div id="bo_list">
<div class="sched-calendar theme-box">
...
<colgroup>
  ...
  <tr>
	...
	<td>
		...
		<i class="ico nextmth"><?php echo $month_pre ?>월</i>
		</a>
		<?}?>
	</td>
	<td>
...
<table border="0" class="theme-list sched-list">
<thead>
  <tr align="center">     
	<th class="sun">S</th>
	<th>M</th>
...
$query = "SELECT * FROM {$write_table} WHERE wr_1!='' and left(wr_1,6) <= '$year$sel_mon' and (wr_2='' or left(wr_2,6) >= '$year$sel_mon') ORDER BY wr_id ASC";
$result = sql_query($query);
$j=0; // layer id
// 내용을 보여주는 부분
while ($row = sql_fetch_array($result)) {  // 제목글 뽑아서 링크 문자열 만들기..
 if(!$is_admin && strstr($row['wr_option'],'secret')) continue;
  ...
  if(!$row['wr_2']){
	$end_day = $start_day;
  } else if( substr($row['wr_2'],0,6) >  $year.$sel_mon ) {
  ...
  for ($i = $start_day ; $i <= $end_day;  $i++) {
	...
	$starter="liner"; 
	if($i==$start_day) $starter.=" starter";
	if($i==$end_day) $starter.=" ender";
	if($d==0) $starter.=" first";
	if($d==6) $starter.=" last";

	$liner="";
	$style="";
	if(strstr($starter,"starter")){
		if($w_e==$w) $liner=$d_e-$d+1;
		else $liner=7-$d;
	}
	...
	if($member['mb_level']>=$board['bo_read_level']) $link=G5_BBS_URL."/board.php?bo_table=".$bo_table."&wr_id=".$row['wr_id'];
	else $link="javascript:void(0)";
    $html_day[$i].= '<a href="'.$link.'" class="txt-default '.$starter.'" data-day="'.$d.'" data-week="'.$w.'" data-idx="'.$j.'" '.$style.' >';
	...
	if(!$row['wr_2'] || $start_day!=$end_day) $html_day[$i].=' ~ ';
	if($row['wr_2'] && $start_day!=$end_day) $html_day[$i].=date("Y.m.d",strtotime($row['wr_2']));
	...
	if (($is_admin||!strstr($row['wr_option'],'secret'))&& $row['wr_id'] != $sc_id && date('Ymd', strtotime($row['wr_1'])) <= date('Ymd') && date('Ymd', strtotime($row['wr_2'])) >= date('Ymd')) {
		$today_schedule .= "<div class='today-schedule'><p><span class='ico ".$row['wr_3']."'></span> <strong>".$row['wr_subject']."</strong>";
		$today_schedule .= " (".date("m/d", strtotime($row['wr_1']));
		if($row['wr_2']) $today_schedule.=" ~ ".date("m/d",strtotime($row['wr_2']));
		$today_schedule.=")</p>";
		if($row['wr_10']) $today_schedule .= "<p class='sched-content'>".$row['wr_10']."</p>";
		$today_schedule .="</div>";
	}
  ...
  for ($iz = 1; $iz <= $lastcount; $iz++) { // 42번을 칠하게 된다.
	...
	*/
	if($write_href){
		echo '<a href="'.$write_href.'&fr_date='.$fr_date.'" class="txt-default date"><i>'.$daytext.'</i></a>';
	} else {
		echo '<i>'.$daytext.'</i>';
	}
	echo $html_day[$cday];
