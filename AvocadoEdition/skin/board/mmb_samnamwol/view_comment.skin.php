...
<div class="<?=$cmt_depth>0 ? "item-reply ":"";?><?=($has_child['wr_id']&&($has_child['wr_id']!=$log_comment['wr_id'])) ? "parent " : "";?><?=$has_child['wr_id']==$log_comment['wr_id'] ? "last ": "";?>item-comment" id="c_<?php echo $comment_id ?>" style="padding-left:<?=$cmt_depth?>px;">
	...
	<div class="co-footer">
		<!--삭제-->
		<?php if ($log_comment['is_del'])  { ?><a href="<?php echo $log_comment['del_link'];  ?>" onclick="return comment_delete();" class="del"></a><?php } ?>
		<!--수정-->
		<?php if ($log_comment['is_edit']) { ?><a href="<?php echo $c_edit_href;  ?>" onclick="comment_box('<?=$list_item['wr_id']?>','<?php echo $comment_id ?>', 'cu'); return false;" class="mod"></a><?php } ?>
		<!--답변-->
		<? if ($log_comment['is_reply'] && $log_comment['wr_is_comment']!=0) { ?><a href="<? echo $c_reply_href;  ?>" onclick="comment_box('<?=$list_item['wr_id']?>','<? echo $comment_id ?>', 'c'); return false;" class="re"></a><? } ?>
		<span class="date">
			<?=date('Y-m-d', strtotime($log_comment['wr_datetime']))?>
		</span>	
	</div>
